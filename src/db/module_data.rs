...
pub fn upsert_module_data(connection: &mut PgConnection, module_data: &ModuleData) -> anyhow::Result<()> {
...
pub fn get_module_module_data(connection: &mut PgConnection, module_identifier_val: &str) -> anyhow::Result<Vec<ModuleData>> {
    let module_data = moduledata.filter(module_identifier.eq(module_identifier_val)).select(ModuleData::as_select()).load(connection)?;
